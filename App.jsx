...
function RegForm({ t, lang, remaining, audition, regs, onRegistered }) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (error) { setBusy(false); return setErr(mapErr(error.message)); }
...
function MyPassages({ t, lang }) {
  ...
  async function verify() {
    ...
    setBusy(false);
    if (error) { setMe(null); return setErr((error.message || "").includes("too_many_attempts") ? t.errTooMany : t.cancelNo); }
    setMe({ name: name.trim(), code: code.trim() }); setPassages(data || []);
  }
  ...
  async function saveEdit(p) {
    ...
    const built = valid.map((w) => ({ composer: w.composer.trim(), text: w.text.trim(), sec: (Number(w.min) || 0) * 60 + (Number(w.sec) || 0) }));
    const { error } = await supabase.rpc("edit_registration", { p_reg_id: p.id, p_name: me.name, p_pin: me.code, p_works: built });
    if (error) return setEditErr(mapErr(error.message));
    setEditing(null); setMsg(t.editSaved); refresh();
  }
  async function doDelete() {
    const { error } = await supabase.rpc("cancel_registration", { p_reg_id: confirmDel.rid, p_name: me.name, p_pin: delCode.trim() });
    if (error) return setDelErr(mapErr(error.message));
