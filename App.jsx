...
const T = {
  fr: {
    ...
    errClosedNow: "Les inscriptions pour cette audition ne sont pas ouvertes en ce moment.",
    errGeneric: "Une erreur est survenue. Réessaie dans un instant.",
    errParticipantOver: "Un passage ne peut pas dépasser 20 minutes.",
    errOverGeneric: "La durée totale dépasse le temps disponible pour cette audition.",
    errTooMany: "Trop de tentatives. Réessaie dans un quart d’heure.",
    sending: "Envoi…",
  ...
  en: {
    ...
    errClosedNow: "Registration for this audition is not open right now.",
    errGeneric: "Something went wrong. Please try again in a moment.",
    errParticipantOver: "A single slot cannot exceed 20 minutes.",
    errOverGeneric: "The total duration exceeds the time available for this audition.",
    errTooMany: "Too many attempts. Please try again in about 15 minutes.",
    sending: "Sending…",
...
const PARTICIPANT_MAX_SEC = 1200; // 20 min max par participant
...
export default function App() {
  ...
  // Charge auditions + inscriptions + accompagnateurs depuis la base.
  async function reload() {
    const [aRes, rRes, acRes] = await Promise.all([
      supabase.from("auditions").select("*").order("starts_at"),
      supabase.from("registrations").select("*"),
      supabase.from("accompanists_public").select("*"),
    ]);
    setAuditions((aRes.data || []).map((a) => ({ id: a.id, title: a.title, date: a.starts_at, location: a.location, limit: a.limit_min })));
    const grouped = {};
    for (const r of rRes.data || []) (grouped[r.audition_id] ||= []).push({
      id: r.id, name: r.display_name, works: r.works || [], accompanist: r.accompanist,
      score: r.score_type, pdf: r.pdf_path ? { name: "partition.pdf", url: null, path: r.pdf_path } : null,
      pages: r.pages || [], validatedBy: r.validated_by,
    });
    setRegs(grouped);
    setAccompanists((acRes.data || []).map((a) => ({ id: a.id, name: a.name })));
  }
  useEffect(() => { reload(); }, []);
    ...
    <Frame>
      ...
      <main style={{ padding: "28px 18px 60px" }}>
        <div style={{ maxWidth: 680, margin: "0 auto" }}>
          ...
          {view === "audition" && <Detail t={t} lang={lang} audition={auditions.find((a) => a.id === sel)} regs={regs[sel] || []} accompanists={accompanists} onRegistered={reload} back={() => go("home")} />}
          {view === "cancel" && <MyPassages t={t} lang={lang} />}
          {view === "acc" && <AccSpace t={t} lang={lang} accompanists={accompanists} />}
          {view === "admin" && <Admin t={t} lang={lang} auditions={auditions} regs={regs} accompanists={accompanists} reload={reload} />}
        </div>
      </main>
    </Frame>
  );
...
/* ───────────────────────────── Détail ───────────────────────────────────── */
function Detail({ t, lang, audition, regs, accompanists, onRegistered, back }) {
  ...
  return (
    <div className="fade">
      ...
      <div style={{ marginTop: 30 }}>
        {st.s === "open" ? <RegForm t={t} lang={lang} remaining={st.remaining} audition={audition} regs={regs} onRegistered={onRegistered} />
...
/* ───────────────────────────── Formulaire ───────────────────────────────── */
function RegForm({ t, lang, remaining, audition, regs, onRegistered }) {
  ...
  const [pstatus, setPstatus] = useState({ has_code: false, penalized: false });
  const [busy, setBusy] = useState(false);

  // Interroge la base : cette personne a-t-elle déjà un code ? est-elle pénalisée ?
  useEffect(() => {
    const nm = name.trim();
    if (!nm) { setPstatus({ has_code: false, penalized: false }); return; }
    let stop = false;
    const id = setTimeout(async () => {
      const { data } = await supabase.rpc("person_status", { p_name: nm });
      if (!stop && data) setPstatus(data);
    }, 350);
    return () => { stop = true; clearTimeout(id); };
  }, [name]);

  const total = works.reduce((s, w) => s + (Number(w.min) || 0) * 60 + (Number(w.sec) || 0), 0);
  const existing = pstatus.has_code;
  const pen = pstatus.penalized;
  ...
  const reset = () => { setDone(null); setName(""); setWorks([{ composer: "", text: "", min: "", sec: "" }]); setPin(""); setPin2(""); setAcc(false); setScore("paper"); setPdf(null); setPages([{ from: "", to: "" }]); if (onRegistered) onRegistered(); };
  ...
  function handleFile(e) { const f = e.target.files && e.target.files[0]; if (f) setPdf({ name: f.name, file: f }); }
  function mapErr(m) {
    m = m || "";
    if (m.includes("already_registered")) return t.errAlready;
    if (m.includes("too_many_attempts")) return t.errTooMany;
    if (m.includes("locked")) return t.errTooMany;
    if (m.includes("wrong_code")) return t.codeWrong;
    if (m.includes("weak_code")) return t.codeWeak;
    if (m.includes("bad_code")) return t.codeBad;
    if (m.includes("code_mismatch")) return t.codeMismatch;
    if (m.includes("no_duration")) return t.errDur;
    if (m.includes("name_required")) return t.errName;
    if (m.includes("over_budget")) return t.errOver(fmtDur(remaining), fmtDur(total));
    if (m.includes("participant_over")) return t.errParticipantOver;
    if (m.includes("frozen") || m.includes("not_open_yet")) return t.errClosedNow;
    return t.errGeneric;
  }
  async function submit() {
    ...
    if (total > PARTICIPANT_MAX_SEC) return setErr(t.errParticipantOver);
    if (!/^\d{4}$/.test(pin.trim())) return setErr(t.codeBad);
    if (!existing && pin.trim() !== pin2.trim()) return setErr(t.codeMismatch);
    setErr(""); setBusy(true);
    try {
      let pdfPath = null;
      if (acc && score === "pdf" && pdf && pdf.file) {
        const path = crypto.randomUUID() + ".pdf";
        const up = await supabase.storage.from("scores").upload(path, pdf.file, { contentType: "application/pdf" });
        if (up.error) { setBusy(false); return setErr(t.errGeneric); }
        pdfPath = path;
      }
      ...
      const { data, error } = await supabase.rpc("register", {
        p_audition: audition.id, p_display_name: name.trim(), p_works: built,
        p_pin: pin.trim(), p_pin2: pin2.trim(),
        p_accompanist: acc, p_score_type: acc ? score : null,
        p_pdf_path: pdfPath, p_pages: acc && score === "pdf" ? pages.filter((p) => p.from && p.to) : [],
      });
      if (error || !data) { setBusy(false); return setErr(t.errGeneric); }
      if (data.status !== "ok") { setBusy(false); return setErr(mapErr(data.status)); }
      setDone({ name: name.trim(), works: built, total, isNew: !existing });
    } catch (e) {
      setErr(t.errGeneric);
    } finally {
      setBusy(false);
  ...
  return (
    <Card>
      ...
      <button style={{ ...btn(), opacity: blocked || busy ? 0.45 : 1, cursor: blocked || busy ? "not-allowed" : "pointer" }} disabled={blocked || busy} onClick={submit}>{busy ? t.sending : t.submit}</button>
    </Card>
  );
}

/* ─────────────────────── Mes passages (consulter / modifier / annuler) ───── */
function MyPassages({ t, lang }) {
  const [name, setName] = useState(""); const [code, setCode] = useState("");
  const [me, setMe] = useState(null); const [passages, setPassages] = useState([]);
  ...
  function mapErr(m) {
    m = m || "";
    if (m.includes("participant_over")) return t.errParticipantOver;
    if (m.includes("over_budget")) return t.errOverGeneric;
    if (m.includes("frozen")) return t.errClosedNow;
    if (m.includes("no_duration")) return t.errDur;
    if (m.includes("too_many_attempts")) return t.errTooMany;
    if (m.includes("locked")) return t.errTooMany;
    if (m.includes("wrong_code")) return t.codeWrong;
  ...
  async function verify() {
    ...
    setBusy(true);
    const { data, error } = await supabase.rpc("my_passages", { p_name: name.trim(), p_pin: code.trim() });
    setBusy(false);
    if (error || !data) { setMe(null); return setErr(t.cancelNo); }
    if (data.status === "locked") { setMe(null); return setErr(t.errTooMany); }
    if (data.status !== "ok") { setMe(null); return setErr(t.cancelNo); }
    setMe({ name: name.trim(), code: code.trim() }); setPassages(data.passages || []);
  }
  async function refresh() {
    const { data } = await supabase.rpc("my_passages", { p_name: me.name, p_pin: me.code });
    if (data && data.status === "ok") setPassages(data.passages || []);
  }
  function startEdit(p) {
    setMsg(""); setEditErr(""); setEditing(p.id);
    setDraft((p.works || []).map((w) => ({ composer: w.composer || "", text: w.text, min: String(Math.floor(w.sec / 60)), sec: String(w.sec % 60) })));
  }
  ...
  async function saveEdit(p) {
    ...
    if (newSec > PARTICIPANT_MAX_SEC) return setEditErr(t.errParticipantOver);
    const built = valid.map((w) => ({ composer: w.composer.trim(), text: w.text.trim(), sec: (Number(w.min) || 0) * 60 + (Number(w.sec) || 0) }));
    const { data, error } = await supabase.rpc("edit_registration", { p_reg_id: p.id, p_name: me.name, p_pin: me.code, p_works: built });
    if (error || !data || data.status !== "ok") return setEditErr(mapErr(data ? data.status : ""));
    setEditing(null); setMsg(t.editSaved); refresh();
  }
  async function doDelete() {
    const { data, error } = await supabase.rpc("cancel_registration", { p_reg_id: confirmDel.rid, p_name: me.name, p_pin: delCode.trim() });
    if (error || !data || data.status !== "ok") return setDelErr(mapErr(data ? data.status : ""));
    setConfirmDel(null); setDelCode(""); setMsg(t.cancelOk(me.name)); refresh();
  }
  ...
  if (!me) return (
    <Card className="fade">
      ...
      <button style={{ ...btn(), opacity: busy ? 0.45 : 1 }} disabled={busy} onClick={verify}>{busy ? t.sending : t.consult}</button>
    </Card>
  );
  ...
  return (
    <Card className="fade">
      ...
      {passages.length === 0
        ...
        : passages.map((p) => {
          const locked = now >= new Date(p.starts_at).getTime() - FREEZE * DAY;
          const isEd = editing === p.id;
          return (
            <div key={p.id} style={{ padding: "16px 0", borderTop: `1px solid ${C.line}` }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", gap: 10 }}>
                <div style={{ fontFamily: serif, fontSize: 17, color: C.ink }}>{p.title}</div>
                <span style={{ fontFamily: sans, fontSize: 12, color: C.grey, whiteSpace: "nowrap" }}>{fmtShort(p.starts_at, lang)}</span>
              </div>

              {!isEd && (p.works || []).map((w, j) => (
              ...
              {isEd && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ display: "flex", gap: 10 }}>
                    ...
                    <button style={btn()} onClick={() => saveEdit(p)}>{t.saveEdits}</button>
                  </div>
                </div>
              )}

              {!isEd && (locked
                ? <div style={{ fontFamily: sans, fontSize: 12.5, color: C.closed, lineHeight: 1.5, marginTop: 8 }}>{t.cancelFrozen(fmtShort(p.starts_at, lang))}</div>
                : <div style={{ display: "flex", gap: 10, marginTop: 10 }}>
                    <button style={{ ...btn("ghost"), padding: "8px 14px", fontSize: 12 }} onClick={() => startEdit(p)}>{t.editBtn}</button>
                    <button style={{ ...btn("danger"), padding: "8px 14px", fontSize: 12 }} onClick={() => { setConfirmDel({ rid: p.id }); setDelCode(""); setDelErr(""); }}>{t.unregister}</button>
                  </div>)}
            </div>
          );
        })}
...
function AccSpace({ t, lang, accompanists }) {
  const [code, setCode] = useState(""); const [err, setErr] = useState(""); const [me, setMe] = useState(null);
  const [requests, setRequests] = useState([]); const [busy, setBusy] = useState(false);

  async function enter() {
    setErr(""); setBusy(true);
    const { data, error } = await supabase.rpc("accompanist_requests", { p_code: code.trim() });
    setBusy(false);
    if (error || !data) return setErr(t.passWrong);
    setMe(data.me); setRequests(data.requests || []);
  }
  async function refresh() {
    const { data } = await supabase.rpc("accompanist_requests", { p_code: code.trim() });
    if (data) setRequests(data.requests || []);
  }
  async function openPdf(path) {
    const w = window.open("", "_blank"); // ouvre l'onglet sur le clic (tablettes), puis on le dirige
    const { data } = await supabase.storage.from("scores").createSignedUrl(path, 300);
    if (data && data.signedUrl && w) w.location = data.signedUrl;
    else if (w) w.close();
  }
  async function setTake(regId, take) {
    await supabase.rpc("accompanist_set", { p_code: code.trim(), p_reg_id: regId, p_take: take });
    refresh();
  }
  ...
  if (!me) {
    return (
      <Card className="fade">
        ...
        {err && <div style={{ fontFamily: sans, fontSize: 13.5, color: C.closed, marginBottom: 12 }}>{err}</div>}
        <button style={{ ...btn(), opacity: busy ? 0.45 : 1 }} disabled={busy} onClick={enter}>{busy ? t.sending : t.enter}</button>
      </Card>
    );
  }
  ...
  return (
    <Card className="fade">
      <div style={eyebrow}>{t.nav_acc}</div>
      <div style={{ fontFamily: serif, fontSize: 18, color: C.ink, margin: "6px 0 2px" }}>{me.name}</div>
      ...
      {requests.length === 0 ? <p style={{ fontFamily: sans, fontSize: 14, color: C.grey }}>{t.accNone}</p> :
        requests.map((r, i) => (
          <div key={r.id} style={{ padding: "13px 0", borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
            <div style={{ fontFamily: serif, fontWeight: 500, fontSize: 17, color: C.ink }}>{r.display_name}</div>
            <div style={{ fontFamily: sans, fontSize: 12.5, color: C.grey, margin: "2px 0 6px" }}>{r.title} · {(r.works || []).map((w) => (w.composer ? w.composer + ", " : "") + w.text).join(" / ")}</div>
            {r.score_type === "pdf" ? (
              <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
                {r.pdf_path
                  ? <button onClick={() => openPdf(r.pdf_path)} style={{ ...btn("ghost"), padding: "8px 14px", fontSize: 12 }}>{t.accView}</button>
                  : <span style={{ fontFamily: sans, fontSize: 12.5, color: C.p700, fontStyle: "italic" }}>{t.accPdf}</span>}
            ...
            <div style={{ marginTop: 8 }}>
              {r.validated_by === me.id
                ? <span style={{ fontFamily: sans, fontSize: 12.5, color: C.ok }}>{t.validatedYou} · <button onClick={() => setTake(r.id, false)} style={{ border: "none", background: "none", color: C.closed, cursor: "pointer", fontFamily: sans, fontSize: 12.5, padding: 0 }}>{t.undo}</button></span>
                : r.validated_by
                  ? <span style={{ fontFamily: sans, fontSize: 12.5, color: C.grey, fontStyle: "italic" }}>{t.takenBy((accompanists.find((x) => x.id === r.validated_by) || {}).name)}</span>
                  : <button onClick={() => setTake(r.id, true)} style={{ ...btn(), padding: "8px 14px", fontSize: 12 }}>{t.validate}</button>}
            </div>
          </div>
        ))}
    </Card>
...
function Admin({ t, lang, auditions, regs, accompanists, reload }) {
  const [admin, setAdmin] = useState(null);
  const [passInput, setPassInput] = useState(""); const [gateErr, setGateErr] = useState(""); const [gateBusy, setGateBusy] = useState(false);
  ...
  async function login() {
    ...
    const { data, error } = await supabase.rpc("admin_people", { p_admin: passInput.trim() });
    setGateBusy(false);
    if (error) return setGateErr(t.passWrong);
    setAdmin(passInput.trim()); setPeople(data || []);
  }
  async function refreshPeople() {
    const { data } = await supabase.rpc("admin_people", { p_admin: admin });
  ...
  if (!admin) return (
    <Card className="fade">
      ...
      <p style={{ fontFamily: sans, fontSize: 14, color: C.grey, margin: "6px 0 16px" }}>{t.adminLead}</p>
      <Field label={t.passLabel}><input style={input} type="password" value={passInput} onChange={(e) => setPassInput(e.target.value)} /></Field>
      {gateErr && <div style={{ fontFamily: sans, fontSize: 13.5, color: C.closed, marginBottom: 12 }}>{gateErr}</div>}
      <button style={{ ...btn(), opacity: gateBusy ? 0.45 : 1 }} disabled={gateBusy} onClick={login}>{gateBusy ? t.sending : t.enter}</button>
    </Card>
  );

  const penalizedSet = new Set(people.filter((p) => p.penalized).map((p) => p.key));

  async function create() {
    if (!title.trim() || !date || !limit) return;
    const { error } = await supabase.rpc("admin_create_audition", { p_admin: admin, p_title: title.trim(), p_starts_at: new Date(date).toISOString(), p_location: loc.trim(), p_limit_min: Number(limit) });
    if (!error) { setTitle(""); setDate(""); setLoc(""); setLimit(""); reload(); }
  }
  const delAudition = (id) => supabase.rpc("admin_delete_audition", { p_admin: admin, p_id: id }).then(reload);
  const delRegistration = (rid) => supabase.rpc("admin_delete_registration", { p_admin: admin, p_reg_id: rid }).then(reload);
  const setCode = (name, pin) => supabase.rpc("admin_set_code", { p_admin: admin, p_name: name, p_pin: pin }).then(refreshPeople);
  const setPenalty = (name, on) => supabase.rpc("admin_set_penalty", { p_admin: admin, p_name: name, p_on: on }).then(refreshPeople);
  const addAcc = (name, code) => supabase.rpc("admin_add_accompanist", { p_admin: admin, p_name: name, p_code: code }).then(reload);
  const delAcc = (id) => supabase.rpc("admin_delete_accompanist", { p_admin: admin, p_id: id }).then(reload);
  ...
  return (
    <Card className="fade">
        ...
        return (
          <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "11px 0", borderTop: `1px solid ${C.line}` }}>
            ...
            <div style={{ display: "flex", gap: 10, alignItems: "center", flexShrink: 0 }}>
              ...
              <button onClick={() => ask(t.confirmDelAudition(a.title), () => delAudition(a.id))} style={{ border: "none", background: "none", color: C.closed, cursor: "pointer", fontFamily: sans, fontSize: 12.5 }}>{t.deleteA}</button>
            </div>
          </div>
        );
      })}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        ...
        <button style={{ ...btn(), whiteSpace: "nowrap" }} onClick={() => { if (accName.trim() && accCode.trim()) { addAcc(accName.trim(), accCode.trim()); setAccName(""); setAccCode(""); } }}>{t.accAdd}</button>
      </div>
      {accompanists.map((a) => (
        <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderTop: `1px solid ${C.line}` }}>
          <span style={{ fontFamily: sans, fontSize: 14, color: C.ink }}>{a.name}</span>
          <button onClick={() => ask(t.confirmDelAcc(a.name), () => delAcc(a.id))} style={{ border: "none", background: "none", color: C.closed, cursor: "pointer", fontFamily: sans, fontSize: 12.5 }}>{t.deleteA}</button>
        </div>
      ))}
      ...
      {[...auditions].filter((a) => new Date(a.date).getTime() < now).sort((a, b) => new Date(b.date) - new Date(a.date)).map((a) => (
        <div key={a.id} style={{ marginBottom: 16, paddingTop: 4, borderTop: `1px solid ${C.line}` }}>
          ...
          {(regs[a.id] || []).length === 0
            ...
            : (regs[a.id] || []).map((r) => {
              const k = norm(firstOf(r.name)); const on = penalizedSet.has(k);
              return (
                <label key={r.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "7px 0", cursor: "pointer" }}>
                  <input type="checkbox" checked={on} onChange={() => setPenalty(firstOf(r.name), !on)} style={{ width: 17, height: 17, accentColor: C.closed }} />
                  <span style={{ fontFamily: sans, fontSize: 13.5, color: on ? C.closed : C.ink }}>{r.name} <span style={{ color: C.grey }}>— {t.noShow}</span></span>
                </label>
              );
            })}
        </div>
          ...
          {(regs[a.id] || []).length === 0
            ...
            : (regs[a.id] || []).map((r) => (
              <div key={r.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 0" }}>
                ...
                <button onClick={() => ask(t.confirmDelReg(r.name), () => delRegistration(r.id))} style={{ border: "none", background: "none", color: C.closed, cursor: "pointer", fontFamily: sans, fontSize: 12.5 }}>{t.removeReg}</button>
              </div>
            ))}
        </div>
      ))}
      ...
      {people.map((p) => {
        const val = codeEdits[p.key] ?? "";
        return (
          <div key={p.key} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, padding: "8px 0", borderTop: `1px solid ${C.line}` }}>
            <span style={{ fontFamily: sans, fontSize: 13.5, color: C.ink, minWidth: 0 }}>{p.name} {!p.has_code && <span style={{ color: C.closed, fontSize: 12 }}>· {t.noCodeYet}</span>}</span>
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexShrink: 0 }}>
              <input style={{ ...input, width: 92, letterSpacing: ".2em" }} inputMode="numeric" maxLength={4} placeholder="••••" value={val} onChange={(e) => setCodeEdits((s) => ({ ...s, [p.key]: e.target.value.replace(/\D/g, "") }))} />
              <button style={{ ...btn("ghost"), padding: "8px 12px", fontSize: 11.5, opacity: /^\d{4}$/.test(val) ? 1 : 0.4 }} onClick={() => { if (/^\d{4}$/.test(val)) ask(t.confirmSetCode(p.name), () => { setCode(p.name, val); setCodeEdits((s) => ({ ...s, [p.key]: "" })); }); }}>{t.save}</button>
            </div>
          </div>
        );
      })}
      <div style={{ display: "flex", gap: 8, marginTop: 14, flexWrap: "wrap" }}>
        ...
        <button style={{ ...btn(), whiteSpace: "nowrap" }} onClick={() => { if (ncName.trim() && /^\d{4}$/.test(ncVal)) ask(t.confirmSetCode(ncName.trim()), () => { setCode(ncName.trim(), ncVal); setNcName(""); setNcVal(""); }); }}>{t.codeSetBtn}</button>
      </div>
    </Card>
  );
}
